...
export class NeedleGraphPanel {
    ...
    private constructor(
        ...
        // MUST register before setting html so we don't miss the webview's first message
        ...
        // Push proactively too. VS Code buffers messages to a webview until it's
        // ready, so this does not depend on the 'ready' handshake arriving.
        this.sendGraphData();
    }
    ...
    private async sendGraphData() {
        try {
            ...
            await this._panel.webview.postMessage({ type: 'graphData', graph, status });
        } catch (e) {
            const message = e instanceof Error ? e.message : String(e);
            await this._panel.webview.postMessage({ type: 'graphError', message });
        }
    }
...
<meta http-equiv="Content-Security-Policy"
  content="default-src 'none'; script-src ${d3Uri} 'nonce-${nonce}'; style-src 'unsafe-inline';">
<style>
...
.kb{display:inline-block;padding:1px 6px;border-radius:10px;font-size:10px;margin-bottom:8px}
...
.fv a{color:var(--vscode-textLink-foreground);cursor:pointer;text-decoration:none}
...
.overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:14px;background:var(--vscode-editor-background,#0c0c0d);font-size:13px;color:var(--vscode-descriptionForeground);text-align:center;padding:24px}
.overlay.hidden{display:none}
.oi{font-size:28px;opacity:.5}
.om{line-height:1.7;max-width:360px}
.oc{font-family:var(--vscode-editor-font-family,monospace);font-size:12px;background:rgba(127,127,127,.15);border:1px solid var(--vscode-panel-border,#333);border-radius:4px;padding:5px 14px}
.rb{background:#7C3AED;color:#fff;border:none;border-radius:4px;padding:6px 18px;font:inherit;font-size:12px;cursor:pointer}
.rb:hover{background:#6d28d9}
</style>
</head>
<body>
<div class="toolbar">
...
<div class="main">
  <div id="canvas">
    <div class="overlay" id="overlay">
      <div class="oi">⟳</div>
      <div class="om" id="om">Loading graph…</div>
    </div>
    <svg id="svg"></svg>
  </div>
  <div id="detail"></div>
...
let ALL_N=[],ALL_E=[],sim,svg,g,lSel,nSel,tSel;
let filters=new Set(['function','method','class','struct','trait','endpoint','module']);
let term='';

// Tell extension we are ready — fires synchronously when this script runs
vscode.postMessage({ type: 'ready' });

window.addEventListener('message', e => {
  const m = e.data;
  if (m.type === 'graphData') {
    const {nodes,edges,stats} = m.graph;
    ALL_N = nodes||[]; ALL_E = edges||[];
    document.getElementById('stats').textContent =
      (stats.total_nodes||0)+' nodes · '+(stats.total_edges||0)+' edges · '+(m.status.total_files||0)+' files';
    buildFilters(stats||{});
    document.getElementById('overlay').classList.add('hidden');
    render();
  }
  if (m.type === 'graphError') {
    const ov = document.getElementById('overlay');
    ov.innerHTML = '<div class="oi">⚠</div><div class="om">'+esc(m.message)+'</div><div class="oc">needle serve</div><button class="rb" id="rb">Retry</button>';
    document.getElementById('rb').onclick = () => {
      ov.innerHTML = '<div class="oi">⟳</div><div class="om" id="om">Retrying…</div>';
...
function buildFilters(s) {
  ...
  [['endpoint','Endpoints ('+(s.endpoints||0)+')'],['class','Classes ('+(s.classes||0)+')'],
   ['function','Functions ('+(s.functions||0)+')'],['method','Methods ('+(s.methods||0)+')'],
   ['module','Modules ('+(s.modules||0)+')']]
  .forEach(([k,l])=>{
    const lb=document.createElement('label');
    lb.innerHTML='<input type="checkbox" value="'+k+'"'+(filters.has(k)?' checked':'')+'>'+l;
    lb.querySelector('input').onchange=e=>{ if(e.target.checked) filters.add(k); else filters.delete(k); rerender(); };
    w.appendChild(lb);
  });
}
...
function rerender() {
  ...
  const ve=ALL_E.filter(e=>e.kind!=='contains'&&vi.has(e.from)&&vi.has(e.to));
  const deg={};
  ve.forEach(e=>{deg[e.from]=(deg[e.from]||0)+1;deg[e.to]=(deg[e.to]||0)+1;});
  const el=document.getElementById('svg');
  const W=el.clientWidth||900, H=el.clientHeight||600;
  if(sim) sim.stop();
  sim=d3.forceSimulation(vn)
    .force('link',d3.forceLink(ve).id(d=>d.id).distance(60).strength(0.5))
    .force('charge',d3.forceManyBody().strength(-120))
    .force('center',d3.forceCenter(W/2,H/2))
    .force('col',d3.forceCollide().radius(d=>r(d,deg)+4));
  g.selectAll('*').remove();
  lSel=g.append('g').attr('opacity',0.4).selectAll('line').data(ve).join('line')
    .attr('stroke',d=>d.kind==='imports'?'#6366f1':'#555').attr('stroke-width',1).attr('marker-end','url(#arr)');
  nSel=g.append('g').selectAll('circle').data(vn).join('circle')
    .attr('r',d=>r(d,deg)).attr('fill',d=>COLORS[d.kind]||'#888')
    .attr('stroke','#1a1a1a').attr('stroke-width',1.5).attr('cursor','pointer')
    .on('click',(_,d)=>pick(d,deg,ve))
    .call(d3.drag()
      .on('start',(e,d)=>{if(!e.active)sim.alphaTarget(0.3).restart();d.fx=d.x;d.fy=d.y;})
      .on('drag',(e,d)=>{d.fx=e.x;d.fy=e.y;})
      .on('end',(e,d)=>{if(!e.active)sim.alphaTarget(0);d.fx=null;d.fy=null;}));
  nSel.append('title').text(d=>d.name+' ('+d.kind+')\n'+d.file_path+':'+d.line_start);
  tSel=g.append('g').attr('pointer-events','none')
    .selectAll('text').data(vn.filter(n=>n.kind==='endpoint'||(deg[n.id]||0)>=6)).join('text')
    .attr('font-size',9).attr('fill','#ccc').attr('text-anchor','middle')
    .attr('dy',d=>-(r(d,deg)+3)).text(d=>d.name.length>24?d.name.slice(0,22)+'…':d.name);
  sim.on('tick',()=>{
...
function pick(node,deg,edges) {
  const dt=document.getElementById('detail'); dt.classList.add('open');
  const c=COLORS[node.kind]||'#888';
  ...
  dt.innerHTML='<h3>'+esc(node.name)+'</h3><span class="kb" style="background:'+c+'22;color:'+c+'">'+esc(node.kind)+'</span>'+
    ...
    (node.detail?'<div class="field"><div class="fl">Detail</div><div class="fv">'+esc(node.detail)+'</div></div>':'')+
  ...
  dt.querySelector('#ol')?.addEventListener('click',()=>vscode.postMessage({type:'openFile',path:node.file_path,line:node.line_start}));
  dt.querySelectorAll('.cl').forEach(a=>a.addEventListener('click',()=>{
    const t=ALL_N.find(n=>n.id===+a.dataset.id);
    if(t){const d={};ALL_E.forEach(e=>{d[e.from]=(d[e.from]||0)+1;d[e.to]=(d[e.to]||0)+1;});pick(t,d,ALL_E);focus(t);}
  }));
}

function focus(n) {
  if(!svg||n.x===undefined) return;
  const el=document.getElementById('svg');
  svg.transition().duration(400).call(d3.zoom().transform,
    d3.zoomIdentity.translate(el.clientWidth/2-n.x,el.clientHeight/2-n.y));
}

document.getElementById('fitBtn').addEventListener('click',()=>{
  if(!svg||!g) return;
  const el=document.getElementById('svg'), b=g.node().getBBox();
  const W=el.clientWidth, H=el.clientHeight;
  const sc=0.85/Math.max(b.width/W,b.height/H);
  svg.transition().duration(400).call(d3.zoom().transform,
    d3.zoomIdentity.translate(W/2-sc*(b.x+b.width/2),H/2-sc*(b.y+b.height/2)).scale(sc));
});
document.getElementById('browserBtn').addEventListener('click',()=>vscode.postMessage({type:'openBrowser'}));
